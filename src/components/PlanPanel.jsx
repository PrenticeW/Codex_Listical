...
import { CHIP_EDITOR_GROUPS } from '../utils/chipEditorColours';
...
// Chip editor event — fired by TacticsPage (right-click menu pencil icons) to
// open the chip-definition editor view in this panel.
// detail: { kind: 'default'|'project'|'custom', id, name, colour, chipId? }
// The edited result is sent back via PLAN_PANEL_ACTION_EVENT ('commitChipEditor').
export const PLAN_PANEL_CHIP_EDITOR_EVENT = 'plan-panel-chip-editor';
...
const TRAY_INSET = 7;
// Keep in sync with PanelShell's scaleBaseWidth default — the width past which
// the tray zooms its content instead of reflowing it.
const PANEL_SCALE_BASE_WIDTH = 420;
...
// ─── Chip editor view (chip definitions: default / project / custom) ──────────
// Ported from the Plan page's old right-click edit sub-view; opens via
// PLAN_PANEL_CHIP_EDITOR_EVENT and commits in one action on Confirm.

// ChipEditorView is shared now — extracted to src/components/ChipEditorView.jsx
// so the Manage Statuses panel reuses the exact same editor.
...
export default function PlanPanel() {
  ...
  // View width must match the tray's LAYOUT width, not the panel's rendered
  // width. Above PanelShell's scaleBaseWidth (420), the tray content keeps its
  // base-width layout and is magnified via CSS zoom — so views must lay out at
  // the base width or they overflow the tray and the px-based slide offset
  ...
  // Chip-definition editor target — pushed in via PLAN_PANEL_CHIP_EDITOR_EVENT
  // (right-click menu pencil icons on the Plan page)
  const [chipDefEditor, setChipDefEditor] = useState(null);
  // Whether the panel was already open when the editor was requested — if not,
  // closing the editor closes the panel again rather than leaving it stranded.
  const wasOpenBeforeEditorRef = useRef(false);
  ...
  useEffect(() => {
    ...
    window.addEventListener(PLAN_PANEL_CHIP_EDITOR_EVENT, handler);
    return () => window.removeEventListener(PLAN_PANEL_CHIP_EDITOR_EVENT, handler);
  }, [isOpen, open, openView]);
