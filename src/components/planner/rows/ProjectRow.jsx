...
export default function ProjectRow({
  ...
  // Get project label (use nickname if available, otherwise full name)
  const projectLabel = projectNickname || projectName;

  // Get weekly quota for this project (only for header rows)
  const rawQuota = isHeader ? (projectWeeklyQuotas.get(projectLabel) ?? 0) : null;
  ...
  return (
    <>
      ...
      {row.getVisibleCells().map(cell => {
        ...
        if (isDayColumn) {
          cellContent = '\u00A0';
        } else if (columnId === 'checkbox' && isHeader) {
          // Project name in checkbox column for header rows
          cellContent = projectLabel;
        } else if (columnId === 'task' && !isHeader) {
          // Section label in task column for section rows
          ...
          >
            <div
              ...
              style={{
                ...
                paddingLeft: columnId === 'checkbox' || columnId === 'task' ? '8px' : '3px',
                paddingRight: '3px',
                fontWeight: isHeader && (columnId === 'checkbox' || columnId === 'estimate' || columnId === 'timeValue') ? '600' : '400',
              }}
            >
              {cellContent}
            </div>
