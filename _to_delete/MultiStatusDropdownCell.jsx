...
function MultiStatusDropdownCell({
  ...
  onShownInstanceChange, // (dayIndex | null) => void — day cell to focus-ring while the panel is open
}) {
  const instances = getMultiInstances(rowData, totalDays);
  const currentIdx = getCurrentInstanceIndex(instances);

  const [open, setOpen] = useState(autoOpen);
  const [shownIdx, setShownIdx] = useState(currentIdx);
  ...
  // it rests on the current instance (first non-terminal, else last).
  const trigger = open ? instances[safeShownIdx] : instances[currentIdx];
  const triggerIdx = open ? safeShownIdx : currentIdx;
  ...
  const toggle = () => {
    if (!open) setShownIdx(currentIdx); // opening resets to the current instance
    setOpen((v) => !v);
  };
  ...
  const handleSelect = (option) => {
    // Manually setting 'Scheduled' is blocked everywhere (system-only status)
    if (option === 'Scheduled') return;
      ...
      {open && createPortal(
        ...
        >
          ...
          {DROPDOWN_OPTIONS.map((option, i) => {
            ...
            const isBlocked = option === 'Scheduled';
            return (
              <div
                key={option}
                ...
                style={{
                  ...
                  background: i === hovered && !isBlocked ? '#f3f4f6' : 'transparent',
                }}
              >
                <div
                  style={{
                    ...
                    opacity: isBlocked && !isCurrent ? 0.45 : 1,
