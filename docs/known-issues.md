...
| Chip identity is positional — stable schedule-item IDs deferred | Chip ids are `schedule-chip-{projectId}-{itemIdx}`, where `itemIdx` is the item's position in the project's Goal-page schedule list. Deleting, inserting mid-list, or reordering schedule items renumbers every item below the change, so the System page sees the surviving chips as new. Mitigated 2026-08-30 in `ProjectTimePlannerV2.jsx`: a Step 0 re-link pass rematches orphaned rows to same-group (project + chip name) chips before Step 1 deletes anything, and rows holding user data that can't be rematched are kept flagged `_chipOrphaned` (greyed out in `TaskRow`/`ProjectRow`) instead of deleted. The re-link fails on a rename combined with a renumber in the same edit — the row is then kept-but-orphaned rather than re-linked. Proper fix: mint a permanent id per schedule item on the Goal page, thread it through `planSummary.scheduleItems` → `buildScheduleLayout` → chip ids, with a one-time migration of existing chip ids, System rows, chip notes, and `chipTimeOverrides` keys. |
...
| Bug | Detail |
|---|---|
| `recurring` stored as two different value vocabularies | Chip task rows are written with `recurring: 'Recurring'` by the System page's sync insert paths but canonicalised to `'true'` by `resetSubprojectLabels` on Send to System, and consumers disagree: `archiveYear.js` and `useFilteredData.js` test `=== 'Recurring'` while `TaskRowPanel`, `useComputedDataV2` and `useEditState` test `=== 'true'`. A Send therefore flips unedited rows to `'true'`, after which year archiving and the recurring filter no longer treat them as recurring. Fix: pick one vocabulary (a `normalizeValue`-style helper already exists in `valueNormalizers.ts`), normalise on read, and migrate stored rows. |
...
**Fourth hole (2026-09-08): a stale browser showed a week hidden on another device.** The stale-tab refetch above only covered `planner_rows`. `usePlannerStorage` skips the network entirely on a cache hit, so `planner_settings` (`visible_day_columns` = hidden weeks, week names, toggles, sort sets, sizing) came from the localStorage mirror for the whole session, and the next Hide/Show Week press would have written that stale map back. Fix: `readPlannerSettingsRowStrict` (throws on failure, never returns "all visible" defaults) is called on a cache-hit mount, on `planner-rows-stale`, and on focus/pageshow (30s throttle); values are adopted only when they differ; `markPlannerRowsStale` also drops the settings cache; and settings autosave (`settingsOpts`) is gated until the mount-time revalidation settles.

**Same day — archived weeks "opening on their own".** `useCollapsibleGroups` re-saved whatever it loaded on every mount (a stale browser therefore wrote its old `collapsed_groups` over the server's), and `readCollapsedGroups` returned an empty Set on any failure (auth not ready, network blip) which was then saved — expanding every archived week on every device. Now: the hook reads server truth via `readPlannerSettingsRowStrict`, keeps cached state on failure, only persists a set that differs from the last server-adopted/saved baseline, and revalidates on `planner-rows-stale` and focus/pageshow. `readCollapsedGroups` remains for other callers but the hook no longer uses it.

**Fifth hole (2026-09-08, localhost wipe): an empty in-memory row state under a still-authorised session deleted the whole year.** The diff save's delete permission (known ids + `basedOnAt` basis) lives in module state in `storage.js`, while the desired rows live in page state — the two can desync. If the page's rows are ever rebuilt as (near-)empty while the earlier read's basis still stands — a failed/blank read that still flips `storageLoaded` (the hydration effect sets `dataHydrated` true even for empty `taskRows`), a Vite HMR remount mid-session, or the `findYearId` miss that used to CACHE `[]` and poison every later read — the next autosave read as "user deleted everything" and emptied `planner_rows`. `archived_weeks` (separate table) survived, so the archive week headers rendered with nothing nested, and chip-injected rows were re-created from Plan — the observed symptom. Fixes: (1) wipe circuit breaker in `_saveTaskRowsImpl` — deletes are refused (upserts still land, `[planner-save] wipe circuit breaker` on console) when the desired state lost the Inbox structural row the UI always carries, or when the save would delete ≥5 rows and ≥80% of the server's rows at once; (2) the no-year-row read no longer caches `[]`. Tested in `offlineReplay.test.js`. The exact localhost trigger was not reproduced — the breaker closes every path to the delete.
...
| `supabase/migrations/20260922000002_raise_min_client_build.sql` | Raises the write-gate minimum build to `20260922` (the order-key builds), locking pre-fix web bundles and pre-fix mobile builds out of destructive planner writes | **Applied to the live DB 2026-09-23**, after the 20260922 web deploy and TestFlight install. (`20260922000001_add_planner_order_key.sql` was applied 2026-09-22.) |
...
| `_importNeedsSubprojectReview` amber treatment | `TaskRow.jsx` styles rows carrying this flag (3px amber left edge + 5% amber wash) and `useEditState.ts` clears it when project/subproject is edited, but nothing in the codebase ever sets it — it has never rendered. Added April 2026 ("Turnover" commit), setter never wired; likely superseded by the reconcile that clears `subproject` instead. Either wire a setter in the import flow or remove both halves. The grey `_chipOrphaned` treatment (2026-08-30) reuses the visual pattern but is live and independent. |
